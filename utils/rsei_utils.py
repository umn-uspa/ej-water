...
def load_rsei_flowlines(site='Onsite', year=2022, selection_aoi=None, universe='core01'):
    ...
    if selection_aoi is not None:
        ...
        rsei_flowlines = rsei_flowlines.sjoin(
            selection_aoi.to_crs(rsei_flowlines.crs),
...
def aggregate_RSEI(years, selection_aoi=None,  universe='core01'):
    ...
    for year in years:
        ...
        rsei_onsite = load_rsei_flowlines(site='Onsite', year=year, selection_aoi=selection_aoi, universe=universe)
        rsei_offsite = load_rsei_flowlines(site='Offsite', year=year, selection_aoi=selection_aoi, universe=universe)
        ...
        rsei_combined = rsei_all_releases.dissolve(['COMID', 'REACHCODE'], aggfunc='sum').reset_index()
        rsei_list.append(rsei_combined)
    # average over the 5-year study period
    rsei_all_years = pd.concat(rsei_list)
    rsei_aggregated = rsei_all_years.dissolve(['COMID', 'REACHCODE'], aggfunc='sum').reset_index()
    rsei_aggregated['TOXCONC'] = rsei_aggregated['TOXCONC']/5
    return rsei_aggregated

def get_source_facilities(site, tabular_gm, aoi=None, universe='core01', years=range(2008,2023)):
    releases, submissions, facilities, naics, _ = load_facility_data()
    all_contributing_facs = []
    fac_columns = ["FacilityID", "FacilityName", "Latitude", "Longitude", "NAICS1",
                   "Street", "City", "County", "State", "ZIPCode"]
    ind_columns = ["2022NAICSCode", "TRIIndustrySector", "LongName"]
    for year in years:
        # read flowlines
        flowlines = load_rsei_flowlines(site=site, year=year, selection_aoi=aoi, universe=universe)
        ...
        selected = tabular_gm_annual.set_index(["ComID", "ReachCode"]).loc[key_tuples].reset_index()
        # join tabular GM with facility-level data
        joined = selected[["ReleaseNumber", "ToxConc"]].merge(releases[["ReleaseNumber", "SubmissionNumber"]], on="ReleaseNumber")
        joined = joined.merge(submissions[["SubmissionNumber", "FacilityID"]], on="SubmissionNumber")
        joined = joined.merge(facilities[fac_columns], on="FacilityID")
        joined = joined.merge(naics[ind_columns], left_on="NAICS1", right_on="2022NAICSCode")
        #print ("Got {0} unique facilities in {1}".format(len(joined["FacilityID"].unique()), year))
        contributing_facs = joined.groupby(fac_columns+ind_columns).sum().reset_index()
    ...
    print ("Got {} unique facilities for the study period".format(len(all_contributing_facs["FacilityID"].unique())))
    all_contributing_facs = all_contributing_facs[fac_columns+ind_columns+['ToxConc']].groupby(fac_columns+ind_columns).sum().reset_index()
    all_contributing_facs.drop(columns=['NAICS1'], inplace=True)
    all_contributing_facs = all_contributing_facs.sort_values(by="ToxConc", ascending=False)
    return all_contributing_facs

def get_source_chemicals(site, tabular_gm, aoi=None, universe='core01', years=range(2008,2023)):
    ...
    for year in years:
        ...
        # join tabular GM with facility-level data
        joined = selected[["ReleaseNumber", "ToxConc"]].merge(releases[["ReleaseNumber", "SubmissionNumber"]], on="ReleaseNumber")
        joined = joined.merge(submissions[["SubmissionNumber", "ChemicalNumber"]], on="SubmissionNumber")
    ...
    print ("Got {} unique chemicals for the study period".format(len(all_contributing_chems["ChemicalNumber"].unique())))
    all_contributing_chems = all_contributing_chems[chem_columns+['ToxConc']].groupby(chem_columns).sum().reset_index()
    all_contributing_chems = all_contributing_chems.sort_values(by="ToxConc", ascending=False)
