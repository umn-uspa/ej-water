"""Compute group-level weighted exposure and disparity ratios."""
...
def compute_weighted_exposure_stats(
    table, study_period="2018_2022", metric="2018_2022_TOXCONC"
):
    """Return weighted exposure and disparity ratios by demographic group."""
    ...
    for var_name, var_name_long in var_names.items():
        weighted = (
            table[f"{study_period}_{var_name}"]
            * table[f"{study_period}_total"]
            * table[metric]
        ).sum()
        indicator = (
            weighted
            / (
                table[f"{study_period}_{var_name}"] * table[f"{study_period}_total"]
            ).sum()
        )
    ...
    result["Disparity Ratio"] = result["Weighted Exposure"] / result[
        "Reference Group"
    ].map(ref_exposure)
    return result
